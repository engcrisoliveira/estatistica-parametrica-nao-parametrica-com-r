...
library(readr)
tensao_emocional <- read_xlsx("tensao_emocional.xlsx")
df <- tensao_emocional
...
df2 <- df %>% 
  gather(key = "tipo_emocao", value = "tensao", Medo, Alegria, Tristeza, Calma) %>% 
  convert_as_factor(id, tipo_emocao)
...
estatisticas_resumo <- df2 %>% 
  group_by(tipo_emocao) %>% 
  get_summary_stats(sujeito, type = "full")
estatisticas_resumo
...
#4. Visualização ------
df2 %>% 
  ggboxplot(x = "tipo_emocao", y = "tensao", add = "jitter", bxp.errorbar = TRUE)
...
resposta_friedman <- df2 %>% 
  friedman_test(tensao ~ tipo_emocao | id)
resposta_friedman
...
#Tamanho do efeito
df2 %>% 
  friedman_effsize(tensao ~ tipo_emocao | id)

#Resultado: effsize = 0.269
#Ou seja, o tamanho do efeito é pequeno

#6. Comparações múltiplas -----
teste_post_hoc <- df2 %>% 
  wilcox_test(tensao ~ tipo_emocao, paired = TRUE, p.adjust.method = "bonferroni")
teste_post_hoc

#7. Resultados -----
teste_post_hoc <- teste_post_hoc %>% 
  add_xy_position(x = "tipo_emocao")

teste_post_hoc$y.position <- c(7, 6.6, 6.5)

ggboxplot(df2, x = "tipo_emocao", 
          y = "tensao", 
          add = "jitter",
          bxp.errorbar = TRUE) +
  ...
  labs(
    ...
    x = "Tipo de emoção", y = "Tensão (s)"
  )

#Resultado:x²(3) = 6.45, p = 0.092, n = 8

#Exercício 2: Velocidade dos Atletas -----
